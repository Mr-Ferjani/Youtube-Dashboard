# -*- coding: utf-8 -*-
"""
Created on Tue Feb  7 13:01:57 2023
...
#Retrieving stats of the last 12 months to compare with the stats of the last 6 months using median
df_agg_diff = df_agg.copy()
thresh =  df_agg_diff['Video publish time'].max() - pd.DateOffset(months = 12)
median_agg = df_agg_diff[df_agg_diff['Video publish time'] >= thresh].median(numeric_only=True)

#Calculating the Mean Absolute Deviation for metrics 
numeric_cols = np.array((df_agg_diff.dtypes == 'float64') | (df_agg_diff.dtypes == 'int64'))
df_agg_diff.iloc[:,numeric_cols] = (df_agg_diff.iloc[:,numeric_cols] - median_agg).div(median_agg)

#merge current date data with publish data to get delta in number of days to perform comparison between videos
...
#Building Dashboard
#to run streamlit, open another prompt: streamlit run dashboard.py
...
if add_sidebar == 'Aggregate Metrics':
    ...
    for i in metric_medians6mo.index:
        with columns[count]:
            ...
            count += 1
            if count >= 5:
                #Count is set to 0 to start listing metrics in the second Row
                count = 0
    ...
    #Coloring negative values with red and positive values with green
    #Formatting the columns using format
...
if add_sidebar == 'Individual Video Analysis':
    #st.write('Ind')
    vids = tuple(df_agg['Video title'])
    video_select = st.selectbox('Pick a video', vids)
    ...
    #Now passing figure into streamlit plotly to show up in the dashboard
    st.plotly_chart(fig)
